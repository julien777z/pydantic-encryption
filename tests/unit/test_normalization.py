...
class TestCombined:
    def test_whitespace_then_lowercase(self):
        assert normalize_value("  Hello  World  ", strip_whitespace=True, normalize_to_lowercase=True) == "hello world"

    def test_strip_non_digits_then_no_case_effect(self):
        result = normalize_value("Phone: +1-555-0100", strip_non_digits=True, normalize_to_lowercase=True)
